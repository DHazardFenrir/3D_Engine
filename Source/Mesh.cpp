...
void Mesh::Load(const tinygltf::Model& model, const tinygltf::Mesh& mesh)
{
	...
	for (const auto& primitive : mesh.primitives)
	{
		const auto& itPos = primitive.attributes.find("TEXCOORD_0");
		if (itPos != primitive.attributes.end())
		{
			const tinygltf::Accessor& posAcc = model.accessors[itPos->second];
			...
			float2* ptr = reinterpret_cast<float2*>(glMapBufferRange(GL_ARRAY_BUFFER, 0, sizeof(float) * 2 * vertexCount, GL_MAP_WRITE_BIT));
			for (size_t i = 0; i < posAcc.count; ++i)
			{
				ptr[i] = *reinterpret_cast<const float2*>(bufferPos);

				bufferPos += posView.byteStride == 0 ? sizeof(float) * 2 : posView.byteStride;
